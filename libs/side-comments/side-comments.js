...
SideComments.prototype.initialize = function( existingComments ) {
  _.each($('.comments-container .side-comment'), function(sideCommentElem){
    $(sideCommentElem).remove();
  });
  _.each(this.$el.find('.commentable-section'), function( section ){
    ...
    console.log(_.find(this.sections, {id: sectionId}));
...
SideComments.prototype.insertComment = function( comment ) {
  var section;
  if(comment.oldSectionId || comment.oldSectionId == 0){
      section = _.find(this.sections, { id: parseInt(comment.oldSectionId) });
  }
  else{
      section = _.find(this.sections, { id: parseInt(comment.sectionId) });
  }
   var element = section.insertComment(comment);
  // if(!this.commentsMap){
  //   this.commentsMap = [];
  // }
  // console.log(element);
  // this.commentsMap.push(element.find('.side-comment').offset());
  // this.commentsMap.forEach(function(mapValue){
  //   if(mapValue.top == element.find('.side-comment').offset().top){
  //     element.find('.side-comment').offset({top: element.find('.side-comment').offset().top, left: element.find('.side-comment').offset().left - 30 })
  //   }
  // })
 ...
 */
SideComments.prototype.removeComment = function(commentId ) {
  var sectionId = parseInt($($('li[data-comment-id=' + commentId + ']').closest('.side-comment')[0]).attr('data-relative-id'));
...
SideComments.prototype.bodyClick = function( event ) {
  ...
  if ($target.closest('.side-comment').length < 1 && $target.closest('body').length > 0 && (!event || !event.target || event.target.id != "showSideComment")) {
    if ($('.side-comment.active').length) {
        $('.side-comment.active').each(function(){
...
SideComments.prototype.bodyChange = function( event ) {
  console.log(event);
}
...
SideComments.prototype.destroy = function() {
  this.hideComments();
  $(document).off(this.clickEventName, '.side-comment.active .add-comment');
	$(document).off(this.clickEventName, '.side-comment.active .post');
	$(document).off(this.clickEventName, '.side-comment.active .cancel');
	$(document).off(this.clickEventName, '.side-comment.active .delete');
	$(document).off(this.clickEventName, '.side-comment.active .modify');
  $(document).off(this.clickEventName, '.side-comment.active .update');
};
...
function Section( eventPipe, $el, currentUser, comments ) {
	...
	//$(document).on(this.clickEventName, '.side-comment .marker', _.bind(this.markerClick, this));
	$(document).on(this.clickEventName, '.side-comment.active .add-comment', _.bind(this.addCommentClick, this));
	$(document).on(this.clickEventName, '.side-comment.active .post', _.bind(this.postCommentClick, this));
	$(document).on(this.clickEventName, '.side-comment.active .cancel', _.bind(this.cancelCommentClick, this));
	$(document).on(this.clickEventName, '.side-comment.active .delete', _.bind(this.deleteCommentClick, this));
	$(document).on(this.clickEventName, '.side-comment.active .modify', _.bind(this.modifyCommentClick, this));
  $(document).on(this.clickEventName, '.side-comment.active .update', _.bind(this.updateCommentClick, this));
	this.render();
}


Section.prototype.checkIsActive = function(){
  return $('.side-comment[data-relative-id=' + this.$el.attr('data-section-id') + ']').hasClass('active');
}
/**
...
Section.prototype.postComment = function() {
  console.log(this.$el);
	var $commentBox = $('.side-comment[data-relative-id=' + this.$el.attr('data-section-id') + ']').find('.comment-box');
...
Section.prototype.insertComment = function( comment ) {
  if(comment.oldSectionId){
    $('.side-comment[data-relative-id=' + comment.oldSectionId + ']').attr('data-relative-id', comment.sectionId);
    this.id = comment.sectionId;
    this.$el.attr('data-section-id', comment.sectionId);
    delete comment.oldSectionId;
  }
  var tempComment = _.find(this.comments, { id: parseInt(comment.id) });
  if(!tempComment || tempComment.length || tempComment == null){
    this.comments.push(comment);
  }
  else{
    tempComment.comment = comment.comment;
    if($('div[data-relative-id=' + comment.sectionId + ']').find('.comments li[data-comment-id=' + comment.id + ']')){
      $('div[data-relative-id=' + comment.sectionId + ']').find('.comments li[data-comment-id=' + comment.id + ']').remove();
    }
  }
	...
	$('div[data-relative-id=' + comment.sectionId + ']').find('.comments').append(newCommentHtml);
	$('div[data-relative-id=' + comment.sectionId  + ']').addClass('has-comments');
...
Section.prototype.modifyCommentClick = function( event ) {
	event.preventDefault();
  if(this.checkIsActive()){
    var commentId = $(event.target).closest('li').data("comment-id");
    var comment = _.find(this.comments, { id: commentId });
    $(event.target).closest('li').find(".update-comment-box").val(comment.comment);
    $(event.target).closest('li').find(".update-controls").show();
    $(event.target).closest('li').find(".comment").hide();
    $(event.target).closest('li').find(".actions").hide();
  }


};

Section.prototype.updateCommentClick = function( event ) {
	event.preventDefault();
  if(this.checkIsActive()){
    var commentId = parseInt($(event.target).closest('li').attr("data-comment-id"));
  	var comment = _.find(this.comments, { id: commentId });
    comment.comment = $(event.target).closest('li').find('.update-comment-box').val();
...
Section.prototype.deleteComment = function( commentId ) {
  $($('li[data-comment-id=' + commentId + ']').closest('.side-comment')[0]).removeClass('active');
...
Section.prototype.removeComment = function( commentId ) {
	...
	$('div[data-relative-id=' + this.$el.attr('data-section-id') + ']').find('li[data-comment-id="'+commentId+'"]').remove();
	this.updateCommentCount();
	if (this.comments.length < 1) {
    $('div[data-relative-id=' + this.$el.attr('data-section-id') + ']').remove();
	}
};
...
Section.prototype.select = function() {
  console.log(this.$el);
	if (this.isSelected()) {
		...
		$('div[data-relative-id=' + this.$el.attr('data-section-id') + ']').addClass('active');
...
Section.prototype.deselect = function() {
		$('div[data-relative-id=' + this.$el.attr('data-section-id') + ']').removeClass('active');
	this.hideCommentForm();
};

Section.prototype.isSelected = function() {
	return 	$('div[data-relative-id=' + this.$el.attr('data-section-id') + ']').hasClass('active');
};
...
Section.prototype.render = function() {
  ...
  console.log(this.$el);
  var topOffset = $(this.$el).offset().top + $('#bookScrolling').scrollTop() - $('.comments-container').offset().top;
  var commentsLineContainer = this.getLineCommentContainer(topOffset);
  this.$el.find('.side-comment').attr('data-relative-id', this.$el.attr('data-section-id'));
  var _this = this;
  this.$el.find('.side-comment').find('.marker').click(function(event){
    _this.markerClick(event);
  });
  var previusElement = null;
  $('.commentable-section').each(function(){
    var topDifference = $(this).offset().top - $(_this.$el).offset().top;
    if(topDifference < 10){
      if($(this).offset().left > $(_this.$el).offset().left){
        previusSectionId = $('[data-relative-id=' + $(this).attr('data-section-id') + ']');
      }
    }
  });
  if(previusElement != null){
    previusElement.after(this.$el.find('.side-comment'));
  }
  else{
    commentsLineContainer.append(this.$el.find('.side-comment'));
  }
  $('.comments-container').append(commentsLineContainer);
};


Section.prototype.getLineCommentContainer = function(topOffset){
    var commentsLineContainer = null;
    $('.comments-line-container').each(function(){
      var topOffsetDifference = (parseInt($(this).css('top')) - topOffset);
      topOffsetDifference = (topOffsetDifference < 0) ? (topOffsetDifference * -1) : topOffsetDifference;
      if(topOffsetDifference < 10){
        commentsLineContainer = $(this);
      }
    });
    if(commentsLineContainer == null){
      commentsLineContainer = $('<div class="comments-line-container"></div>');
    }
    commentsLineContainer.css({top: topOffset + 'px'});
    return commentsLineContainer;
